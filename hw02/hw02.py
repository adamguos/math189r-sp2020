...
def linreg(X, y, reg=0.0):
    ...
    """
    "*** YOUR CODE HERE ***"
    # X corresponds to A in part (b), y corresponds to b, W_opt corresponds to x*
    regulariser = reg * np.eye(X.shape[1])
    regulariser[0, 0] = 0
    coefs = np.matmul(X.T, X) + regulariser
    outs = np.matmul(X.T, y)
    W_opt = np.linalg.solve(coefs, outs)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    RMSE_list = [find_RMSE(W, X_val, y_val) for W in W_list]
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    norm_list = [np.linalg.norm(W) for W in W_list]
...
def linreg_no_bias(X, y, reg=0.0):
    ...
    coefs = (X.T @ X) + (reg * np.eye(n)) - (X.T @ np.ones((m, m)) @ X) / m
    outs = X.T @ (np.eye(m) - np.ones((m, m)) / m) @ y
    W_opt = np.linalg.solve(coefs, outs)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
    ...
    while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
        ...
        "*** YOUR CODE HERE ***"
        train_rmse = np.sqrt(np.sum(np.square(X_train @ W + b - y_train)) / m_train)
        val_rmse = np.sqrt(np.sum(np.square(X_val @ W + b - y_val)) / m_val)

        obj_train.append(train_rmse)
        obj_val.append(val_rmse)

        W_grad = ((X_train.T @ X_train + reg * np.eye(n)) @ W + X_train.T @ (b - y_train)) / m_train
        b_grad = (np.sum(X_train @ W - y_train) + b * m_train) / m_train
...
if __name__ == '__main__':
    ...
    print('\n==> Step 4: Gradient descent')
    b_gd, W_gd = grad_descent(X_train, y_train, X_val, y_val, reg=reg_opt, print_freq=10)
