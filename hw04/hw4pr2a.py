...
def NLL(X, y, W, reg=0.0):
	...
	mu = sigmoid(X @ W)
	summands = y * np.log(mu) + (1 - y) * np.log(1 - mu)
	nll = -1 * np.sum(summands) + reg * (W.T @ W)
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		W_grad = newton_step(X, y, W, reg=reg)
...
def get_description(X, y, W):
	...
	precision = sum([1 if predictions[i] + y[i] == 2 else 0 for i in range(y.shape[0])]) \
		/ np.sum(predictions)
	recall = sum([1 if predictions[i] + y[i] == 2 else 0 for i in range(y.shape[0])]) \
		/ np.sum(y)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	reg_list = [0, 0.01, 0.05, 0.1, 0.5, 1, 5, 10, 30, 50]
	
	for i, reg in enumerate(reg_list):
		print("\nLambda {} out of {}:".format(i + 1, len(reg_list)))
		W, nll_list = grad_descent(X_train, y_train, reg=reg, lr = 2e-4, print_freq=50)
