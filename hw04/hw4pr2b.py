...
def NLL(X, y, W, reg=0.0):
	...
	mu = np.exp(X @ W)
	factors = mu / mu.sum(axis=1).reshape(-1, 1)
	sumlog = (np.log(factors) * y).sum(axis=1)
	NLL = -1 * sum(sumlog) + reg * np.trace(W.T @ W)
...
def grad_softmax(X, y, W, reg=0.0):
	...
	mu = np.exp(X @ W)
	mu_prob = mu / mu.sum(axis=1).reshape(-1, 1)
	grad = X.T @ (mu_prob - y) + reg * W
...
def predict(X, W):
	...
	mu = np.exp(X @ W)
	mu_prob = mu / mu.sum(axis=1).reshape(-1, 1)
	y_pred = np.argmax(mu_prob, axis=1).reshape(-1, 1)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		W, nll_list = grad_descent(X_train, y_train_OH, reg=reg, print_freq=1)
