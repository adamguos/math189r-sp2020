...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		"*** YOUR CODE HERE ***"
		cluster_dists = cdist(X, clusters)	# cluster_dists[i, j] = dist(x[i], clusters[j])
		closest_clusters = np.argmin(cluster_dists, axis=1)

		for j in range(clusters.shape[0]):
			idn = closest_clusters == j
			clusters[j, :] = (X.T @ (idn)).T / np.sum(idn) if np.linalg.norm(idn) != 0 else np.zeros((1, X.shape[1]))

		"*** END YOUR CODE HERE ***"

		# TODO: Calculate cost and append to cost_list
		"*** YOUR CODE HERE ***"
		cost = k_means_cost(X, clusters, closest_clusters)
...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	cost = np.sum([np.linalg.norm(X[i, :] - clusters[label[i], :]) ** 2 for i in range(1, m)])
...
if __name__ == '__main__':
	...
	for k in range(1, 21):
		...
		"*** YOUR CODE HERE ***"
		clusters, labels, cost_list = k_means(X, k)
	...
	"*** YOUR CODE HERE ***"
	plt.scatter(range(1, 21), cost_k_list)
	...
	data_plot = plt.scatter(X[:, 0], X[:, 1], zorder=1)
	center_plot = plt.scatter(clusters[:, 0], clusters[:, 1], zorder=2)
